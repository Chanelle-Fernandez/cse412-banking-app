<?php
	...
	if(isset($_POST['deposit'])) {	
		...
		$sql = "INSERT INTO transaction (account_number) VALUES ($1)";
		$query = pg_query_params($db_conn, $sql, $val_array);

		//updating deposit table
		//$trans_id = pg_query_params($db_conn, "SELECT MAX(transaction_id) FROM transaction");

		//$val_array = array($amount, $trans_id);
		//var_dump($val_array);		

		//$sql = "INSERT INTO transaction (deposited, transaction_id) VALUES ($1, $2)";
		//$query = pg_query_params($db_conn, $sql, $val_array);
